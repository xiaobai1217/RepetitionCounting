...
class ConfSubNet(nn.Module):
    def __init__(self, block=BasicBlock, layers=[2, 2, 2, 2], num_classes=309, pool='avgpool', zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, lambda1=15.0):
        super(ConfSubNet, self).__init__()
        self.pool = pool
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
    ...
    def forward(self, x, v_feat):
        ...
        x1 = torch.nn.functional.normalize(x1,dim=1)*self.lambda1
        v_feat = torch.nn.functional.normalize(v_feat,dim=1)*self.lambda1
        x1 = torch.cat((x1, v_feat),dim=1)
        x1=self.fc(x1)
        # if self.pool == 'avgpool':
        #
        #     x = self.fc(x1)
